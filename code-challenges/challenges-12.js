...
const cvsFiltration = (arr) => {
    // write your code here
    var qualifiedCandidates = arr.filter(candidate => {
        return candidate.yearsOfExperience > 4 && candidate.tech === "JS";
    });

    var result = qualifiedCandidates.map((candidate) => {
        var fullName = candidate.firstName;
...
const skillComparison = (arr1, arr2) => {
    // write your code here
    return arr1.filter((skill) => {
        if (arr2.includes(skill)) {
            return false;
        }
        return true;
    }
    ).concat(arr2.filter((skill) => {
