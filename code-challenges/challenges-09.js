...
const employeesBonus = (arr) => {
    ...
    arr.forEach((employees) => {
        const currentSalary = parseInt(employees.salary);
        let newSalary;
        if (employees.workHours > 8) {
            newSalary = currentSalary + 100;
            employees.salary = newSalary.toString() + "$";
        }
        else {
            newSalary = currentSalary + 50;
            employees.salary = newSalary.toString() + "$";
        }
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    for (let i = 0; i < mouseArray.length; i++) {
        for (let j = 0; j < keyBoardArray.length; j++) {
            totalPrice = mouseArray[i] + keyBoardArray[j];
